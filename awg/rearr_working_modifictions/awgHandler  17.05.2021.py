"""
Created by Alex Alampounti 2020/2021, variously modified
by Stefan Spence for PyDex integration and Vincent Brooks 
for implementation of rearrangment.

Rearrangement: 
    - Rearrangement functions included as methods of AWG class so that they have access
      to the AWG methods. This is not possible if they are instantiated inside a nested class
      or in a seperate class. The other way would be to call the AWG class from a rearrangment 
      class.
...
import rearrange_extra_functions as rxtra
...
class AWG:
    ...
    def __init__ (self, channel_enable = [0,1],sample_rate = MEGA(625), num_segment = int(16) , start_step=int(0)):
        ...
        self.rearrMode = 'use_exact'  # use_exact or use_all. use_exact is simpler but throws out extra atoms or cases with insufficient atoms
    ...
    def load(self,file_dir='Z:\Tweezer\Experimental\AOD\m4i.6622 - python codes\Sequence Replay tests\metadata_bin\\20200819\\20200819_165335.txt'):
        ...
        if self.rearrToggle == False:  # If not rearranging, then everytime you load, reset card base params.
            self.stop()       
        ...
        for i in range(segNumber):
            ...
            if self.rearrToggle == True:
                self.setSegment(i+(self.segmentCounter+1),*tempData)
                self.lastRearrStep = self.segmentCounter+1     # If we hit load some sequence with rearrToggle = True,
            else:                                             # then after rearr instead of going back to step 0, we go to 1st step of   
                self.setSegment(i,*tempData)                   # auxialliary sequence loaded in.
        
        if self.rearrToggle == False:    
    ...
    def calculateAllMoves(self):
        ...
        start_key = rxtra.fstring(self.initial_freqs) # Static array at initial trap freqs 
        ...
        if self.rearrMode == 'use_exact':
            end_key = rxtra.fstring(self.target_freqs) # Static array at target trap freqs
            ...
            if len(self.initial_freqs) < len(self.target_freqs):
                ...
                for x in combinations(start_key, len(self.target_freqs)):
                    self.createRearrSegment(''.join(x)+'m'+''.join(rxtra.fstring(self.target_freqs)))
        ...
        # rearrMode = use_all: ANY atom which is loaded will be rearranged to make as large a complete array as possible.
            ...
            for j in range(len(start_key)):                          # Loop goes through number of sites loaded, e.g. 3/5, 4/5 etc
                ...
                req_n_segs += comb(len(start_key), nloaded) + 1 + 1  # +1 for target static array +1 for target ramping array
            
            # Generate each segment
            for j in range(len(start_key)):
                nloaded = len(start_key)-j   
                end_key = rxtra.fstring(self.target_freqs[:j+1])  # Creat segemtn: static array at target trap freqs
                ...
                for x in combinations(start_key, nloaded):   #  for each of the possible number of traps being loaded
                    self.createRearrSegment(''.join(x)+'m'+''.join(rxtra.fstring([1]*nloaded)))
    ...
    def createRearrSegment(self, key):
        ...
        if 's' in key:
            if 'si' in key:                # Initial array of static traps
                f1 = rxtra.flist(key.partition('s')[0], 'initial')
            elif 'st' in key:              # Target array of static traps
                f1 = rxtra.flist(key.partition('s')[0], 'target')
            data = self.dataGen(self.segmentCounter,
                                ...
                                [self.rearr_freq_amp]*len(f1),         # tone freq. amps
        ...
        # MOVING TRAP    
            ...
            f1 = rxtra.flist(key.partition('m')[0], 'initial') 
            f2 = rxtra.flist(key.partition('m')[2], 'target')
            data = self.dataGen(self.segmentCounter,
                                self.rParam['channel'],
                                'moving',
        ...
        # RAMPING TRAP
            ...
            f2 = rxtra.flist(key.partition('r')[0], 'target')  
    ...
    def calculateSteps(self, occupancyStr):
        ...
        if self.rearrMode == 'use_exact':# Only use cases where AT LEAST the target number of atoms was loaded into the initial array.
            stepKeys =  [''.join(rxtra.fstring(self.initial_freqs))+'si', # n static traps
                        keyStr[-len(self.target_freqs):]+'m'+''.join(rxtra.fstring(self.target_freqs)), # sweeps to fixed # target traps
                        ''.join(rxtra.fstring(self.target_freqs))+'st'] # fixed # of static traps
            
            if len(keyStr) < len(self.target_freqs):   # IF the number of atoms loaded < # target atoms, do NOTHING.
                self.setStep(0, 0, 1, 0, 1)
        
        elif self.rearrMode == 'use_all':
            stepKeys =  [''.join(rxtra.fstring(self.initial_freqs))+'si', # initial number of static traps
                        keyStr+'m'+''.join(rxtra.fstring([1]*len(keyStr))), # number of targets depends on number loaded
                        ''.join(rxtra.fstring([1]*len(keyStr)))+'st'] # number of target static traps depends on number loaded
        ...
        if len(keyStr) == 0:   # If no atoms loaded do nothing.
            ...
            self.setStep(0, segList[0], 1, 1, 1)   # Static traps until TTL received  
            self.setStep(1, segList[1], 1, 2, 2)   # Moving traps for fixed duration, automatically moves to next step   
            self.setStep(2, segList[2], 1, self.lastRearrStep, 1)  # Static traps on at target site until triggered.

            
                    
    def loadRearrParams(self, configdir = r'C:\Users\lldj44\Desktop\pydex_modifications\rearr_working_modifictions'):
        ...
        self.rParam={"amp_adjust":True, 
                     ...
                     "hybridicity":0, 
                     "initial_freqs":[190., 170., 150.], "target_freqs":[190.],
                     "headroom_segs":10, 
                     "rearrMode":"use_all",
                     "rearr_freq_amps":"default",  # If default, rearr freq amps are 1/(n traps). else they are float
                     "power_ramp":False,
                     "final_freq_amp": 1,
                     }
    ...
    def saveRearrParams(self, savedir= r'C:\Users\lldj44\Desktop\pydex_modifications\rearr_working_modifictions'):
        """Save the rearrangement parameters used to a metadata file. """
        with open('rearr_config.json', 'w') as fp:
            json.dump(self.rParam, fp)
    ...
    def printRearrInfo(self):
        ...
        print('Max duration / segment = '+str(4e9/(2*self.num_segment*self.sample_rate.value*1e3*len(self.channel_enable))))+' ms'
    ...
    def setRearrFreqAmps(self, value = 'default'):
        """Set the frequency amplitudes during rearrangment either to default or to some fixed amplitude.
            ...
            """
        if value == 'default':
            self.rearr_freq_amp = 1/len(self.initial_freqs)
        else:
            self.rearr_freq_amp = value

    ...
    def respond(self, cmd=None):
        ...
        if 'save' in cmd:
            ...
            try:
                ...
                if 'args_dict' in params:
                    data = []
                    for chan, vals in params['args_dict'].items():
                        arguments = self.getParams(vals)
                        data.append(self.awg.dataGen(*arguments))
                    self.setSegment(self.stats['segment'], *data)    
                else:
                    arguments = self.getParams(params)
                    data = self.awg.dataGen(*arguments)
                    self.setSegment(self.stats['segment'], data)
            except Exception as e:

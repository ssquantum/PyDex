...
Included modules:
	...
	Single atom image analysis: 1) create histograms, 2) analyse histograms, 3) control settings like the ROI and multirun across histogram producers, 4) check whether atoms were loaded in an ROI and emit a signal that can trigger the experimtn
...
TCP Communication for a DExTer run/multirun:
	...
	2. Python internal: start acquisition
		...
		b. Queue up commands telling DExTer to (1) save the sequence (2) run the sequence (3) confirm it's finished.
			i. In a multirun the sequences are only saved once by python at the very start of the multirun.
...
save_dir	The directory to save trace and graph csv files to	Working directory
n	Run number	0
